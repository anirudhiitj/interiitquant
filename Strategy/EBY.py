...
# This dictionary now holds the *default* configuration.
# DATA_DIR will be overridden by the command-line argument.
EBY_Config = {
    # Data
    'DATA_DIR': './EBY/', # This is now the default
    'NUM_DAYS': 279,
    'OUTPUT_DIR': './',
    'OUTPUT_FILE': 'EBY_signals.csv',
    'TEMP_DIR': './EBY_temp_signals',
    ...
    # Strategy 2 (Pure Momentum/Volatility with Dynamic TP) - Priority 2
    ...
    'P2_TP_MEDIUM': 0.16,       
    'P2_TP_HIGH': 0.29,       
    'P2_STDDEV_TP_LOW': 0.05,     
    'P2_STDDEV_TP_HIGH': 0.08,   
    'P2_TRAILING_STOP_FROM_PEAK': 0.01,
    'P2_CONDITIONAL_SL': 0.1,    
    'P2_KAMA_SLOPE_THRESHOLD': 0.001, 
...
def prepare_p1_features(df: pd.DataFrame, EBY_Config: dict) -> pd.DataFrame:
    # Prepare all derived features 
    
    # KF_Trend
    kf = KalmanFilter(
        transition_matrices=[1],
        observation_matrices=[1],
        transition_covariance=EBY_Config['P1_KF_TRANSITION_COV'],
        observation_covariance=EBY_Config['P1_KF_OBSERVATION_COV'],
        initial_state_mean=df[EBY_Config['PRICE_COLUMN']].iloc[0],
        initial_state_covariance=EBY_Config['P1_KF_INITIAL_STATE_COV']
    )
    filtered_state_means, _ = kf.filter(df[EBY_Config['P1_FEATURE_COLUMN']].dropna().values)
    smoothed_prices = pd.Series(filtered_state_means.squeeze(), index=df[EBY_Config['P1_FEATURE_COLUMN']].dropna().index)
    df["KF_Trend"] = smoothed_prices.shift(1)
    df["KF_Trend_Lagged"] = df["KF_Trend"].shift(1)
    
    df["Take_Profit"] = EBY_Config['P1_TAKE_PROFIT']
    
    # KAMA (primary)
    price = df[EBY_Config['P1_FEATURE_COLUMN']].to_numpy(dtype=float)
    window = EBY_Config['P1_KAMA_WINDOW']
    ...
    sc_fast = 2 / (EBY_Config['P1_KAMA_FAST_PERIOD'] + 1)
    sc_slow = 2 / (EBY_Config['P1_KAMA_SLOW_PERIOD'] + 1)
    ...
    df["KAMA"] = kama
    df["KAMA_Slope"] = df["KAMA"].diff(EBY_Config['P1_KAMA_SLOPE_DIFF'])
    df["KAMA_Slope_abs"] = df["KAMA_Slope"].abs()
    
    # UCM (Unobserved Components Model)
    series = df[EBY_Config['P1_FEATURE_COLUMN']]
    first_valid_idx = series.first_valid_index()
    if first_valid_idx is None:
        raise ValueError(f"{EBY_Config['P1_FEATURE_COLUMN']} has no valid values")
    
    alpha = EBY_Config['P1_UCM_ALPHA']
    beta = EBY_Config['P1_UCM_BETA']
    n = len(series)
...
def generate_p1_signal_single(row, position, cooldown_over, EBY_Config):
    # Generate signal for Kalman Filter strategy (Priority 1)
    signal = 0
    
    if cooldown_over:
        if position == 0:
            if row["KF_Trend"] >= row["UCM"] and row["KAMA_Slope_abs"] > EBY_Config['P1_ENTRY_KAMA_SLOPE_THRESHOLD']:
                signal = 1
            elif row["KF_Trend"] <= row["UCM"] and row["KAMA_Slope_abs"] > EBY_Config['P1_ENTRY_KAMA_SLOPE_THRESHOLD']:
                signal = -1
        elif position == 1:
            if (row["KF_Trend_Lagged"] > row["UCM_Lagged"] and 
                row["KF_Trend"] < row["UCM"] and 
                row["KAMA_Slope_abs"] > EBY_Config['P1_EXIT_KAMA_SLOPE_THRESHOLD']):
                signal = -1
        elif position == -1:
            if (row["KF_Trend_Lagged"] < row["UCM_Lagged"] and 
                row["KF_Trend"] > row["UCM"] and 
...
def generate_p1_signals(df: pd.DataFrame, EBY_Config: dict) -> np.ndarray:
    # Generate Priority 1 (Kalman Filter) signals
    df = prepare_p1_features(df, EBY_Config)
    
    # Initialize state
    position = 0
    entry_price = None
    last_signal_time = -EBY_Config['UNIVERSAL_COOLDOWN']
    signals = [0] * len(df)
    ...
    for i in range(len(df)):
        ...
        price = row[EBY_Config['PRICE_COLUMN']]
        ...
        if is_last_tick and position != 0:
            ...
            cooldown_over = (current_time - last_signal_time) >= EBY_Config['P1_MIN_HOLD_SECONDS']
            signal = generate_p1_signal_single(row, position, cooldown_over, EBY_Config)
...
def generate_p2_signals_internal(
    ...
    for i in range(1, n):
        ...
        if position == 0 and sig != 0:
            ...
            if should_reverse:
                # Exit current position AND reverse
                signals[i] = -position  # Exit signal
                last_direction = position
                
                # Immediate reversal (same bar) - this will be handled by manager
                # We mark both exit and entry on this bar by setting signal to -position
                # The new position will be handled in the next iteration
...
def generate_p2_signals(df: pd.DataFrame, EBY_Config: dict, p4_kama_slope: np.ndarray) -> np.ndarray:
    pb9_values = df[EBY_Config['P2_FEATURE_COLUMN']].values.astype(np.float64)
    prices = df[EBY_Config['PRICE_COLUMN']].values.astype(np.float64)
    ...
    atr = compute_atr_fast(pb9_values, EBY_Config['P2_ATR_PERIOD'])
    stddev = compute_stddev_fast(pb9_values, EBY_Config['P2_STDDEV_PERIOD'])
    momentum = compute_momentum_fast(pb9_values, EBY_Config['P2_MOMENTUM_PERIOD'])
    volatility = compute_volatility_ratio(pb9_values, EBY_Config['P2_VOLATILITY_PERIOD'])
    ...
    signals = generate_p2_signals_internal(
        ...
        atr_thresh=EBY_Config['P2_ATR_THRESHOLD'],
        stddev_thresh=EBY_Config['P2_STDDEV_THRESHOLD'],
        min_hold_seconds=EBY_Config['P2_MIN_HOLD_SECONDS'],
        tp_low=EBY_Config['P2_TP_LOW'],
        tp_medium=EBY_Config['P2_TP_MEDIUM'],
        tp_high=EBY_Config['P2_TP_HIGH'],
        stddev_tp_low=EBY_Config['P2_STDDEV_TP_LOW'],
        stddev_tp_high=EBY_Config['P2_STDDEV_TP_HIGH'],
        trailing_stop=EBY_Config['P2_TRAILING_STOP_FROM_PEAK'],
        cooldown_seconds=EBY_Config['UNIVERSAL_COOLDOWN'],
        conditional_sl=EBY_Config['P2_CONDITIONAL_SL'],
        kama_slope_threshold=EBY_Config['P2_KAMA_SLOPE_THRESHOLD']
    )
...
def generate_p3_signals(df: pd.DataFrame, EBY_Config: dict) -> np.ndarray:
    prices = df[EBY_Config['PRICE_COLUMN']].values.astype(np.float64)
    bb4_values = df[EBY_Config['P3_BB4_COLUMN']].values.astype(np.float64)
    bb6_values = df[EBY_Config['P3_BB6_COLUMN']].values.astype(np.float64)
    pb9_t1_values = df[EBY_Config['P3_VOLATILITY_FEATURE']].values.astype(np.float64)
    pb6_t4_values = df[EBY_Config['P3_V_PATTERN_FEATURE']].values.astype(np.float64)
    timestamps = df['Time_sec'].values.astype(np.float64)
    
    true_range = calculate_true_range_from_pb9(pb9_t1_values)
    v_pattern_atr_values = rolling_mean_numba(true_range, EBY_Config['P3_V_PATTERN_ATR_PERIOD'])
    
    kama = calculate_kama(
        pb9_t1_values,
        EBY_Config['P3_KAMA_PERIOD'],
        EBY_Config['P3_KAMA_FAST'],
        EBY_Config['P3_KAMA_SLOW']
    )
    kama_slope = calculate_kama_slope(kama, EBY_Config['P3_KAMA_SLOPE_LOOKBACK'])
    ...
    signals = generate_p3_signals_internal(
        ...
        kama_slope_threshold=EBY_Config['P3_KAMA_SLOPE_THRESHOLD'],
        v_pattern_atr_threshold=EBY_Config['P3_V_PATTERN_ATR_THRESHOLD'],
        min_hold_seconds=EBY_Config['P3_MIN_HOLD_SECONDS'],
        max_hold_seconds=EBY_Config['P3_MAX_HOLD_SECONDS'],
        cooldown_seconds=EBY_Config['UNIVERSAL_COOLDOWN'],
        use_take_profit=EBY_Config['P3_USE_TAKE_PROFIT'],
        take_profit_offset=EBY_Config['P3_TAKE_PROFIT_OFFSET'],
        strategy_start_bar=EBY_Config['P3_STRATEGY_START_BAR']
    )
...
def prepare_p4_features(df: pd.DataFrame, EBY_Config: dict) -> pd.DataFrame:
    
    # KAMA calculation
    price = df[EBY_Config['P4_FEATURE_COLUMN']].to_numpy(dtype=float)
    window = EBY_Config['P4_KAMA_WINDOW']
    
    kama_signal = np.abs(pd.Series(price).diff(window))
    kama_noise = np.abs(pd.Series(price).diff()).rolling(window=window, min_periods=1).sum()
    er = (kama_signal / kama_noise.replace(0, np.nan)).fillna(0).to_numpy()

    sc_fast = 2 / (EBY_Config['P4_KAMA_FAST_PERIOD'] + 1)
    sc_slow = 2 / (EBY_Config['P4_KAMA_SLOW_PERIOD'] + 1)
    sc = ((er * (sc_fast - sc_slow)) + sc_slow) ** 2
    ...
    df["P4_KAMA"] = kama
    df["P4_KAMA_Slope"] = df["P4_KAMA"].diff(EBY_Config['P4_KAMA_SLOPE_DIFF']).shift(1)
    df["P4_KAMA_Slope_abs"] = df["P4_KAMA_Slope"].abs()

    # ATR calculation
    tr = df[EBY_Config['P4_FEATURE_COLUMN']].diff().abs()
    atr = tr.ewm(span=EBY_Config['P4_ATR_SPAN'], adjust=False).mean()
    df["P4_ATR"] = atr.shift(1)
    df["P4_ATR_High"] = df["P4_ATR"].expanding(min_periods=1).max()

    # Standard Deviation
    std = df[EBY_Config['P4_FEATURE_COLUMN']].rolling(window=20, min_periods=1).std()
...
def generate_p4_signals(df: pd.DataFrame, EBY_Config: dict) -> np.ndarray:
    # Prepare features
    df = prepare_p4_features(df, EBY_Config)
    ...
    for i in range(len(df)):
        ...
        if is_last_tick and position != 0:
            ...
            cooldown_over = (current_time - last_signal_time) >= EBY_Config['P4_MIN_HOLD_SECONDS']
            
            if cooldown_over:
                # Volatility filter
                volatility_confirmed = (row["P4_ATR_High"] > EBY_Config['P4_ATR_THRESHOLD'] and 
                                        row["P4_STD_High"] > EBY_Config['P4_STD_THRESHOLD'])
                
                # Entry conditions
                if position == 0 and volatility_confirmed:
                    if row["P4_KAMA_Slope"] > EBY_Config['P4_KAMA_SLOPE_ENTRY']:
                        signal = 1
                    elif row["P4_KAMA_Slope"] < -EBY_Config['P4_KAMA_SLOPE_ENTRY']:
                        signal = -1
                ...
                # Exit conditions (price-based)
                    ...
                    if position == 1:
                        price_diff = price - entry_price
                        if price_diff >= EBY_Config['P4_TAKE_PROFIT']:
                            signal = -1
                    elif position == -1:
                        price_diff = entry_price - price
                        if price_diff >= EBY_Config['P4_TAKE_PROFIT']:
...
# MANAGER LOGIC (4 PRIORITIES)

def apply_manager_logic(df: pd.DataFrame, p1_signals: np.ndarray, p2_signals: np.ndarray, 
                        p3_signals: np.ndarray, p4_signals: np.ndarray, EBY_Config: dict) -> np.ndarray:
    # Apply manager logic to combine signals from all priorities
    n = len(df)
    ...
    for i in range(n):
        ...
        cooldown_ok = (current_time - last_signal_time) >= EBY_Config['UNIVERSAL_COOLDOWN']
        ...
        # FLAT POSITION
            ...
            if cooldown_ok and not is_last_bar:
                ...
                # Priority 3 entry (only if P1, P2 haven't locked it)
                ...
                # Priority 4 entry (only if P1, P2, P3 haven't locked it)
        ...
        # IN POSITION
            ...
            if position_owner == "P1":
                ...
                # Check if Priority 3 wants to override/reverse
...
def process_single_day(file_path: str, day_num: int, temp_dir: pathlib.Path, EBY_Config: dict) -> dict:
    try:
        # 1. READ DATA
        required_cols = [
            EBY_Config['TIME_COLUMN'],
            EBY_Config['PRICE_COLUMN'],
            EBY_Config['P1_FEATURE_COLUMN'],
            EBY_Config['P2_FEATURE_COLUMN'],
            EBY_Config['P3_BB4_COLUMN'],
            EBY_Config['P3_BB6_COLUMN'],
            EBY_Config['P3_VOLATILITY_FEATURE'],
            EBY_Config['P3_V_PATTERN_FEATURE'],
            EBY_Config['P4_FEATURE_COLUMN'],
        ]
        ...
        df['Time_sec'] = pd.to_timedelta(df[EBY_Config['TIME_COLUMN']].astype(str)).dt.total_seconds().astype(int)
        
        # 2. GENERATE PRIORITY 1 SIGNALS (Kalman Filter)
        p1_signals, df_with_features = generate_p1_signals(df.copy(), EBY_Config)
        
        # 3. GENERATE PRIORITY 4 SIGNALS AND FEATURES (needed for P2)
        p4_signals, df_with_p4_features = generate_p4_signals(df.copy(), EBY_Config)
        p4_kama_slope = df_with_p4_features['P4_KAMA_Slope'].fillna(0).values.astype(np.float64)
        
        # 4. GENERATE PRIORITY 2 SIGNALS (Pure Momentum/Volatility with Dynamic TP and Conditional SL)
        p2_signals = generate_p2_signals(df.copy(), EBY_Config, p4_kama_slope)
        
        # 5. GENERATE PRIORITY 3 SIGNALS (BB Crossover with KAMA and V-pattern)
        p3_signals = generate_p3_signals(df.copy(), EBY_Config)

        # 6. APPLY MANAGER LOGIC
        final_signals = apply_manager_logic(df, p1_signals, p2_signals, p3_signals, p4_signals, EBY_Config)
        
        # 7. COUNT SIGNALS PER STRATEGY
        ...
        output_df = pd.DataFrame({
            EBY_Config['TIME_COLUMN']: df[EBY_Config['TIME_COLUMN']],
            EBY_Config['PRICE_COLUMN']: df[EBY_Config['PRICE_COLUMN']],
...
def main():
    ...
    parser = argparse.ArgumentParser(description="EBY 4-Priority Signal Generator")
    parser.add_argument(
        '--data-dir',
        type=str,
        default=EBY_Config['DATA_DIR'], # Use default from the config dict
    ...
    print("="*80)
    print("STRATEGY MANAGER - 4-PRIORITY SIGNAL ROUTING")
    print("="*80)
    print(f"\nProcessing data from: {EBY_Config['DATA_DIR']}") # Show the directory being used
    print(f"\nPriority 1: Kalman Filter Strategy ({EBY_Config['P1_FEATURE_COLUMN']} based)")
    print(f"  - KF_Trend vs UCM crossovers with KAMA_Slope_abs filter")
    print(f"  - Entry threshold: KAMA_Slope_abs > {EBY_Config['P1_ENTRY_KAMA_SLOPE_THRESHOLD']}")
    print(f"  - Exit threshold: KAMA_Slope_abs > {EBY_Config['P1_EXIT_KAMA_SLOPE_THRESHOLD']}")
    print(f"  - Take Profit: {EBY_Config['P1_TAKE_PROFIT']} absolute")
    print(f"  - Min Hold: {EBY_Config['P1_MIN_HOLD_SECONDS']}s")
    print(f"\nPriority 2: Pure Momentum/Volatility with Dynamic TP ({EBY_Config['P2_FEATURE_COLUMN']} based)")
    print(f"  - Volatility Filter: ATR>{EBY_Config['P2_ATR_THRESHOLD']}, StdDev>{EBY_Config['P2_STDDEV_THRESHOLD']}")
    print(f"  - DYNAMIC TP: Low={EBY_Config['P2_TP_LOW']}, Medium={EBY_Config['P2_TP_MEDIUM']}, High={EBY_Config['P2_TP_HIGH']}")
    print(f"  - PURE TRAILING STOP: Exit when PnL drops {EBY_Config['P2_TRAILING_STOP_FROM_PEAK']} from peak")
    print(f"  - CONDITIONAL SL WITH REVERSAL: SL={EBY_Config['P2_CONDITIONAL_SL']}, KAMA Slope Threshold={EBY_Config['P2_KAMA_SLOPE_THRESHOLD']}")
    print(f"  - StdDev Thresholds: Low<{EBY_Config['P2_STDDEV_TP_LOW']}, High>={EBY_Config['P2_STDDEV_TP_HIGH']}")
    print(f"\nPriority 3: BB Crossover Strategy with KAMA Filter and V-Pattern ({EBY_Config['P3_BB4_COLUMN']}/{EBY_Config['P3_BB6_COLUMN']} based)")
    print(f"  - Starts trading from bar: {EBY_Config['P3_STRATEGY_START_BAR']}")
    print(f"  - KAMA Filter: Period={EBY_Config['P3_KAMA_PERIOD']}, Fast={EBY_Config['P3_KAMA_FAST']}, Slow={EBY_Config['P3_KAMA_SLOW']}")
    print(f"  - KAMA Slope: Lookback={EBY_Config['P3_KAMA_SLOPE_LOOKBACK']}, Threshold={EBY_Config['P3_KAMA_SLOPE_THRESHOLD']}")
    print(f"  - V-PATTERN REVERSAL: Using {EBY_Config['P3_V_PATTERN_FEATURE']}, ATR threshold={EBY_Config['P3_V_PATTERN_ATR_THRESHOLD']}")
    print(f"  - Take Profit: {EBY_Config['P3_TAKE_PROFIT_OFFSET']} absolute")
    print(f"\nPriority 4: KAMA-based Simple Strategy ({EBY_Config['P4_FEATURE_COLUMN']} based)")
    print(f"  - KAMA Slope Entry Threshold: {EBY_Config['P4_KAMA_SLOPE_ENTRY']}")
    print(f"  - Volatility Filter: ATR>{EBY_Config['P4_ATR_THRESHOLD']}, StdDev>{EBY_Config['P4_STD_THRESHOLD']}")
    print(f"  - Take Profit: {EBY_Config['P4_TAKE_PROFIT']} absolute")
    print(f"  - Min Hold: {EBY_Config['P4_MIN_HOLD_SECONDS']}s")
    print(f"\nUniversal Cooldown: {EBY_Config['UNIVERSAL_COOLDOWN']}s between ANY signals")
    print(f"Priority Locking: P1→P2+P3+P4 | P2→P3+P4 | P3→P4")
    ...
    print(f"\nProcessing with {EBY_Config['MAX_WORKERS']} workers...")
    processed_files = []
    total_p1_entries = 0
    total_p2_entries = 0
    total_p3_entries = 0
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"✓ Processed day{result['day_num']:3d} | P1: {result['p1_entries']:3d} | P2: {result['p2_entries']:3d} | P3: {result['p3_entries']:3d} | P4: {result['p4_entries']:3d} | Final: {result['final_entries']:3d}")
                except Exception as e:
