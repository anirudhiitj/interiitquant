...
OUTPUT_DIR = 'V_Feature_Analysis/'  # Directory to save results
...
FEATURE_PATTERN = 'V[!B]*'  # Example: 'BB*' will match all features starting with BB
...
def load_and_analyze_data(file_path, blocksize='256MB'):
    ...
    try:
        ...
        if FEATURES_TO_ANALYZE:
            ...
            import fnmatch
            feature_cols = [f for f in all_feature_cols if fnmatch.fnmatch(f, 'V*') and not f.startswith('VB')]
            print(f"✓ Pattern 'V* (excluding VB*)' matched {len(feature_cols)} features")
        ...
        for part_idx in range(n_partitions):
            ...
            # Extract and convert to numpy (CPU memory)
        ...
        price_valid = np.sum(~np.isnan(price_data))
        print(f"✓ Price data: {price_valid:,}/{total_rows:,} valid points ({100*price_valid/total_rows:.2f}%)")
        
        # Analyze each feature
        print("\n" + "="*100)
        print("ANALYZING CORRELATIONS FOR EACH FEATURE")
        print("="*100)
        ...
        for idx, feature in enumerate(feature_cols, 1):
            ...
            # Count valid points (non-NaN in both price and feature)
            mask = ~(np.isnan(price_data) | np.isnan(feature_data))
            n_valid = np.sum(mask)
            ...
            pearson, _ = gpu_pearson_corr(price_clean, feature_clean)
            spearman = spearman_correlation(price_clean, feature_clean)
            kendall = kendall_correlation(price_clean, feature_clean)
            mi = mutual_information(price_clean, feature_clean)
            best_lag, best_lag_corr = lagged_correlation(price_clean, feature_clean, max_lag=MAX_LAG)
            
            # Granger causality (only for features with enough data)
            if n_valid >= 100:
                print("  Computing Granger causality...")
                granger = granger_causality(feature_clean, price_clean, max_lag=min(5, MAX_LAG))
            else:
                granger = False
            ...
            print(f"  Granger→Price: {'YES ✓' if granger else 'NO'}")
        ...
        csv_file = os.path.join(OUTPUT_DIR, f'correlation_results_{timestamp}.csv')
        summary_df_sorted.to_csv(csv_file, index=False)
        
        # Save as TXT (detailed per-feature report)
        txt_file = os.path.join(OUTPUT_DIR, f'correlation_results_{timestamp}.txt')
        with open(txt_file, 'w', encoding='utf-8') as f:
            ...
            granger_features = summary_df[summary_df['Granger→Price'] == 'YES']
            ...
            for idx, result in enumerate(results, 1):
                ...
                f.write(f"  Granger→Price:     {result['Granger→Price']}\n")
                ...
                if result['Best_Lag'] > 0:
                    f.write(f"  Temporal Pattern: Feature leads price by {result['Best_Lag']} periods\n")
                elif result['Best_Lag'] < 0:
                    f.write(f"  Temporal Pattern: Feature lags price by {abs(result['Best_Lag'])} periods\n")
                else:
                    f.write(f"  Temporal Pattern: Best correlation at zero lag (contemporaneous)\n")
                
                if result['Granger→Price'] == 'YES':
                    f.write(f"  Predictive Power: Feature has predictive power for future price (Granger causality detected)\n")
                else:
        ...
        print("="*100)
        print(summary_df_sorted[['Feature', 'Valid_Pct', 'Pearson', 'Spearman', 'Best_Lag', 'Best_Lag_Corr', 'Granger→Price']].head(10).to_string(index=False))
