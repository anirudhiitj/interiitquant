...
CONFIG = {
    ...
    'OUTPUT_FILE': 'combined_signals_EBX0.csv',
    'TEMP_DIR': '/data/quant14/signals/temp_manager_v7_swap_p2p30',
    ...
    # ========================================================================
    # STRATEGY 1 (KALMAN FILTER) - PRIORITY 1 [UNCHANGED]
    # ========================================================================
    'P1_FEATURE_COLUMN': 'PB9_T1',
    'P1_BLOCK_STD_MIN': 0.15,     # Min Std Dev to block
    'P1_BLOCK_STD_MAX': 0.2,     # Max Std Dev to block
    'P1_BLOCK_WINDOW': 900,     # 30 minutes in seconds
    ...
    # ========================================================================
    # STRATEGY 2 (HAWKES + EAMA) - NOW PRIORITY 3 [KEYS REMAIN P2_]
    # ========================================================================
    'P2_SUPER_SMOOTHER_PERIOD': 25,
    'P2_EAMA_PERIOD': 15,
    'P2_EAMA_FAST_PERIOD': 25,
    'P2_EAMA_SLOW_PERIOD': 100,
    'P2_HAWKES_LOOKBACK': 1500,  #1800
    'P2_HAWKES_THRESHOLD_ABS': 0.00006, #5
    'P2_HAWKES_ALPHA': 1.2,
    'P2_HAWKES_BETA': 1.8,
    ...
    # ========================================================================
    # STRATEGY 3 (MU SPLIT) - NOW PRIORITY 4 [KEYS REMAIN P3_]
    # ========================================================================
    ...
    # High Mu (> 100) Settings (Sigma > 0.12)
    ...
    # ========================================================================
    # STRATEGY 4 (JMA) - NOW PRIORITY 2 [KEYS REMAIN P4_]
    # ========================================================================
    'P4_JMA_LENGTH': 1800,
    'P4_SUPER_SMOOTHER_PERIOD': 30,
    
    # EAMA Slow (Signal Line)
    ...
    'P4_MIN_SIGNAL_GAP': 240, # Seconds
    'P4_FLIP_DELAY': 240,     # Seconds
    'P4_START_TICK': 1800,
}
...
def generate_p1_signals(df: pd.DataFrame, config: dict) -> np.ndarray:
    """Generate Priority 1 signals with 30-min Volatility Block"""
    
    # ------------------------------------------------------------------
    # NEW LOGIC: 30-Minute Volatility Block
    # ------------------------------------------------------------------
    # Filter data for the first 30 minutes (1800 seconds)
    early_data = df[df['Time_sec'] <= config.get('P1_BLOCK_WINDOW', 1800)]
    
    if not early_data.empty:
        # Calculate standard deviation of the price
        std_dev = early_data[config['PRICE_COLUMN']].std()
        
        # Check if std_dev is within the blocking range [0.2, 0.5]
        block_min = config.get('P1_BLOCK_STD_MIN', 0.2)
        block_max = config.get('P1_BLOCK_STD_MAX', 0.5)
        
        if block_min <= std_dev <= block_max:
            # Return empty signals (zeros) and original df
            return np.zeros(len(df), dtype=np.float64), df
    # ------------------------------------------------------------------
...
# ============================================================================
# STRATEGY 3 (VOLATILITY GATED MU SPLIT)
# ============================================================================
...
def generate_mu_split_signals(df: pd.DataFrame, config: dict) -> np.ndarray:
    """Wrapper for Volatility Gated Mu Split Strategy (CONFIG KEYS START P3_)"""
...
def calculate_jma_numba(prices, length):
    """Jurik Moving Average with adaptive smoothing"""
    n = len(prices)
...
def generate_p4_core(timestamps, prices, jma, eama_slow, atr_values, stddev_values,
    ...
    """Core logic for JMA/EAMA Strategy (CONFIG KEYS START P4_)"""
    n = len(prices)
    signals = np.zeros(n, dtype=np.float64)
    eps = 1e-9 
    
    # States: 0=Neutral, 1=Long, 2=LongTrail, 3=Short, 4=ShortTrail, 5=WaitShort, 6=WaitLong
    state = 0 
    ...
    day_activated = False
    pending_entry_type = 0 # 1=Long, -1=Short
    ...
    for i in range(n):
        ...
        # --- LONG MANAGEMENT ---
            ...
            if state == 2: trail_stop = curr_p - trail_points # Update trail in trailing state
...
def generate_p4_signals(df: pd.DataFrame, config: dict) -> np.ndarray:
    """Wrapper for JMA/EAMA Dual Activation (CONFIG KEYS START P4_)"""
...
def apply_manager_logic(df_time_sec, p1_signals, p2_signals, p3_signals, p4_signals, config_cooldown):
    """
    Manager Logic for 4 Priorities. 
    P1 > P2 > P3 > P4
    """
    n = len(df_time_sec)
    ...
    for i in range(n):
        ...
        # --- 1. HANDLE PENDING REVERSAL (Wait Phase) ---
        ...
        # --- 3. EXISTING POSITION (Management & Handovers) ---
        elif abs(position) > EPS and not pending_reversal:
            
            # --- OWNER: P1 (Highest Priority) ---
            if position_owner == 1:
                if cooldown_ok and abs(p1_sig) > 0.01:
                    final_sig = p1_sig; last_signal_time = current_time

            # --- OWNER: P2 (Medium Priority) ---
            elif position_owner == 2:
                # Check P1 (The Interrupter)
                if abs(p1_sig) > 0.01:
            ...
            # --- OWNER: P3 (Low Priority) ---
            elif position_owner == 3:
                # Check P1 (Highest Interrupter)
                if abs(p1_sig) > 0.01:
                    if (p1_sig > 0 and position > 0) or (p1_sig < 0 and position < 0):
                        position_owner = 1; priority_2_locked = True; priority_3_locked = True; priority_4_locked = True; last_signal_time = current_time
                    else:
...
def process_single_day(file_path: str, day_num: int, temp_dir: pathlib.Path, config: dict) -> dict:
    ...
    try:
        ...
        # ----------------------------------------------------------------
        # We calculate these first so the CSV columns are still populated 
        # even if the day is banned (useful for debugging).
        
        p1_signals, df_with_features = generate_p1_signals(df, config) # P1: Kalman
        raw_hawkes_signals = generate_hawkes_signals(df, config)       # P2 Config
        raw_mu_split_signals = generate_mu_split_signals(df, config)   # P3 Config
        raw_jma_signals = generate_p4_signals(df, config)              # P4 Config
        ...
        # ----------------------------------------------------------------
        # MANAGER LOGIC: DAY FILTER (MEAN CROSSOVER)
        # ----------------------------------------------------------------
        # Logic: If expanding mean crosses ABOVE 100.09 between ticks 61 and 300,
        # we disable trading for the day (set final signals to 0), but keep the file.
        
        day_is_banned = False
        
        if len(df) > 300:
            # Get early prices (up to index 300)
            ...
            # Define window: 61 to 300
            # We need to look at i (current) and i-1 (previous)
            # Slice for Current: Indices 61 to 300
            curr_means = expanding_mean.iloc[61:301].values 
            # Slice for Previous: Indices 60 to 299
            prev_means = expanding_mean.iloc[60:300].values 
            
            # Vectorized Crossover Check: 
        ...
        # ----------------------------------------------------------------
        # APPLY MANAGER OR FORCE ZEROS
        # ----------------------------------------------------------------
        
        if day_is_banned:
            # If day is banned, Final Signal is all zeros
            final_signals = np.zeros(len(df), dtype=np.float64)
        else:
            # Otherwise, run the standard priority manager
...
def main(config):
    ...
    print("="*80)
    print("STRATEGY MANAGER - 4-PRIORITY SIGNAL ROUTING [V7 SWAP P2/P3]")
    print("="*80)
    print(f"Priority 1: Kalman Filter Strategy (Feature: {config['P1_FEATURE_COLUMN']})")
    print(f"Priority 2: JMA + EAMA Dual Activation (Formerly P4)")
    print(f"Priority 3: Hawkes Process + EAMA Smoothing (Formerly P2)")
    print(f"Priority 4: Volatility Gated Mu Split (Formerly P3)")
    ...
    try:
            ...
            for result in pool.imap_unordered(process_wrapper, tasks):
                try:
                    if result:
                        ...
                        print(f"✓ Day{result['day_num']:3d} | P1(Kal):{result['p1_entries']:3d} P2(JMA):{result['p2_entries']:3d} P3(Hwk):{result['p3_entries']:3d} P4(Mu):{result['p4_entries']:3d} Final:{result['final_entries']:3d}")
                except Exception as e:
        ...
        print(f"P1 (Kalman):        {total_p1:,}")
        print(f"P2 (JMA):           {total_p2:,}")
        print(f"P3 (Hawkes):        {total_p3:,}")
        print(f"P4 (Mu Split):      {total_p4:,}")
