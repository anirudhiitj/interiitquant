...
Strategy_Config = {
    ...
    'SIGMA_THRESHOLD': 0.11,     # σ threshold
    
    # Take Profit (absolute dollar amounts)
    'TP_LONG': 0.85,   # $0.80 for long
    'TP_SHORT': 0.8,  # $0.75 for short
...
def calculate_direction(df: pd.DataFrame, config: dict) -> int:
    ...
    Entry Rules:
    - Long:  μ > 100 AND σ > 0.11
    - Short: μ < 100 AND σ > 0.11
    ...
    # Entry conditions
    if mean_price > mean_threshold and sigma_price > sigma_threshold:
        return 1  # Long
    elif mean_price < mean_threshold and sigma_price > sigma_threshold:
        return -1  # Short
    else:
        return 0  # No trade
...
def main():
    parser = argparse.ArgumentParser(description="Mean-Sigma Threshold Strategy (50/50 Split)")
    parser.add_argument(
        '--data-dir',
        type=str,
    ...
    parser.add_argument(
        '--sigma-threshold',
        type=float,
        default=Strategy_Config['SIGMA_THRESHOLD'],
        help=f"Sigma threshold for entry. Default: {Strategy_Config['SIGMA_THRESHOLD']}"
    )
    args = parser.parse_args()
    
    Strategy_Config['DATA_DIR'] = args.data_dir
    Strategy_Config['MEAN_THRESHOLD'] = args.mean_threshold
    Strategy_Config['SIGMA_THRESHOLD'] = args.sigma_threshold
    config = Strategy_Config
    
    start_time = time.time()
    
    print("="*80)
    print("MEAN-SIGMA THRESHOLD STRATEGY (50/50 SPLIT)")
    print("="*80)
    ...
    print(f"  Sigma Threshold (σ): {config['SIGMA_THRESHOLD']}")
    ...
    print(f"  Long:  μ > {config['MEAN_THRESHOLD']} AND σ > {config['SIGMA_THRESHOLD']} → Signal = +1.0")
    print(f"  Short: μ < {config['MEAN_THRESHOLD']} AND σ > {config['SIGMA_THRESHOLD']} → Signal = -1.0")
