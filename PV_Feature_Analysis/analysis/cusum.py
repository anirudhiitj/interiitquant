...
def calculate_directional_cusum(PB9_T1s, drift=0.02, decay=0.9):
    """
    ...
    return cusum_up_values, cusum_down_values, PB9_T1_changes
...
def main():
    ...
    # CUSUM Parameters
    DRIFT = 0.001    # Minimum PB9_T1 change to count as directional ($)
    DECAY = 0.95     # Decay factor (e.g., 0.9 to 0.999)
    
    # EWM Dynamic Threshold Parameters
    EWM_SPAN = 180           # EWM Span (similar to a lookback period)
    
    # *** MODIFIED: Split TRENDING_STD_MULT into UP and DOWN ***
    TRENDING_UP_STD_MULT = 1.0   # Multiplier for Trending Up threshold (e.g., 1.0)
    TRENDING_DOWN_STD_MULT = 1.0 # <-- NEW: Set your different multiplier here (e.g., 1.5)
    ...
    print(f"\nCUSUM Parameters:")
    print(f"   Drift: ${DRIFT} (min PB9_T1 move to count)")
    print(f"   Decay: {DECAY} (applied on noise or counter-move)")
    print(f"\nDynamic Classification (EWM):")
    print(f"   EWM Span: {EWM_SPAN} data points")
    
    # *** MODIFIED: Updated print statements for new parameters ***
    ...
    for day_num in days_to_plot:
        ...
        # 1. Calculate CUSUM
        print(f"   Calculating directional CUSUM (no-reset)...")
        PB9_T1s = df['PB9_T1'].values.astype(np.float64)
        cusum_up, cusum_down, PB9_T1_changes = calculate_directional_cusum(
            PB9_T1s, drift=DRIFT, decay=DECAY
        )
        cusum_diff = cusum_up - cusum_down
        
        # 2. Calculate Dynamic EWM Thresholds
        ...
        # *** MODIFIED: Use new TRENDING_DOWN_STD_MULT for the down threshold ***
        ...
        # *** MODIFIED: Replaced JSON output with fig.show() ***
        fig.show()
