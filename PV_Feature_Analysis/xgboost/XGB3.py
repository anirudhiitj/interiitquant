...
def main():
    ...
    # <--- MODIFIED: Path to the TOP 50 feature importance file
    importance_file_path = 'feature_importance_pooled_TOP50.csv'
    
    print("="*80)
    print("POOLED TRAINING (OPTION 1) - TOP 20 FEATURES") # <--- MODIFIED
    print("="*80)
    ...
    print(f"   Method: Loading Top 20 features from '{importance_file_path}'") # <--- MODIFIED
    print(f"\n   === LABELING ===")
    print(f"   Target: $0.25")
    ...
    # ========================================================================
    # <--- MODIFIED BLOCK: LOAD TOP 20 FEATURES --->
    # ========================================================================
    print(f"\n[PRE-4] Loading top 20 features from '{importance_file_path}'...") # <--- MODIFIED
    try:
        imp_df = pd.read_csv(importance_file_path)
    except FileNotFoundError:
        print(f"   ERROR: '{importance_file_path}' not found.")
        print(f"   Please run the 'pooled_TOP50' script first to generate this file.")
        return
    ...
    # <--- MODIFIED: Select the top 20 feature names
    top_20_features = imp_df['Feature'].head(20).tolist()
    print(f"   ✓ Loaded {len(top_20_features)} features for retraining.")
    ...
    # ========================================================================
    # [4/5] POOLED TRAINING AND EVALUATION (TOP 20 FEATURES)
    # ========================================================================
    print(f"\n[4/5] Starting pooled training (TOP 20 FEATURES)...") # <--- MODIFIED
    ...
    # <--- MODIFIED: Only select the Top 20 features + Label
    ...
    print(f"\n   === FINAL POOLED PERFORMANCE (TOP 20) ===") # <--- MODIFIED
    ...
    # ========================================================================
    # [5/5] GET FEATURE IMPORTANCE AND SAVE (FROM TOP 20)
    # ========================================================================
    print(f"\n[5/5] Calculating final feature importance (TOP 20)...") 
    ...
    if total_gain > 0:
        ...
        for feature in feature_cols: # Use the original 20 features
            importance_data.append({
                'Feature': feature,
                'Fractional_Gain': 0.0
            })
    ...
    print(f"\n === FINAL FEATURE IMPORTANCE (Features > Threshold) ===")
    print(f"   {'Feature':<30} | {'Fractional Gain':<18}")
    print("    " + "-" * 50)
    for item in filtered_importance_data: # Print only retained
        print(f"   {item['Feature']:<30} | {item['Fractional_Gain']:<18.4%}")

    # 7. Save the model
    model_path = 'final_pooled_xgb_model_TOP20.json' 
    print(f"\nSaving final model to '{model_path}'...")
    booster.save_model(model_path)

    # 8. Save ALL 20 sorted importance features to CSV
    importance_path = 'feature_importance_pooled_TOP20.csv' 
    print(f"Saving ALL {len(sorted_importance_data)} feature importances to '{importance_path}'...")

    # <--- MODIFIED: Use sorted_importance_data (the full list)
    ...
    print("\n" + "="*80)
    print("✓ TOP 20 POOLED TRAINING & FEATURE IMPORTANCE COMPLETED!") 
    print("="*80)
