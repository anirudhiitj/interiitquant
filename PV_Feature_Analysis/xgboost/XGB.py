...
def main():
    ...
    print("="*80)
    print("POOLED TRAINING (OPTION 1)") # <-- UPDATED
    print("="*80)
    ...
    print(f"\n   === EVALUATION ===")
    print(f"   Method: Pooled 80/20 Train/Test Split (shuffle=False)") # <-- UPDATED
    print(f"   Metric: Final Test Accuracy & Class Recall") # <-- UPDATED
    print("="*80)
    ...
    # ========================================================================
    # [4/5] POOLED TRAINING AND EVALUATION
    # ========================================================================
    print(f"\n[4/5] Starting pooled training...")
    
    # Define feature columns
    first_df_cols = all_labeled_dfs[0].columns
    exclude_cols = ['Label', 'Time', 'timestamp', 'datetime', 'date']
    feature_cols = [c for c in first_df_cols if c not in exclude_cols]
    ...
    df_clean = all_data_df[feature_cols + ['Label']].dropna()
    ...
    # ========================================================================
    # [5/5] GET FEATURE IMPORTANCE AND SAVE (WITH THRESHOLD & CLEAN PRINT)
    # ========================================================================
    print(f"\n[5/5] Calculating final feature importance...")
    ...
    print(f"   Features retained: {len(filtered_importance_data)} / {total_features}")

    # 6. Print final feature importance table (only fractional gain)
    print(f"\n === FINAL FEATURE IMPORTANCE (Top 20, Above Threshold) ===")
    print(f"   {'Feature':<30} | {'Fractional Gain':<18}")
    print("    " + "-" * 50)
    for item in filtered_importance_data[:20]:
        print(f"   {item['Feature']:<30} | {item['Fractional_Gain']:<18.4%}")

    # 7. Save the model
    model_path = 'final_pooled_xgb_model.json' # <-- Renamed model
    print(f"\nSaving final model to '{model_path}'...")
    booster.save_model(model_path)

    # 8. Save filtered importance to CSV
    importance_path = 'feature_importance_pooled.csv' # <-- Renamed importance file
    print(f"Saving filtered feature importance to '{importance_path}'...")

    imp_df = pd.DataFrame(filtered_importance_data)
    if not imp_df.empty:
    ...
    print("\n" + "="*80)
    print("✓ POOLED TRAINING & FEATURE IMPORTANCE COMPLETED!")
    print("="*80)
