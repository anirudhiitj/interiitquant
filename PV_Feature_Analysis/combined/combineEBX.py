...
EBX_Config = {
    ...
    'TEMP_DIR': 'data/quant14/signals/temp_signals',
    ...
    'USE_PRICE_FOR_INDICATORS': True,  # If True, uses Price; if False, uses PB9_T1
    'FEATURE_COLUMN': 'PB9_T1',  # Will be overridden if USE_PRICE_FOR_INDICATORS=True
    ...
    'P1_UCM_ALPHA': 0.8,
    'P1_UCM_BETA': 0.08,
    'P1_ENTRY_KAMA_SLOPE_THRESHOLD': 0.0007,
    'P1_EXIT_KAMA_SLOPE_THRESHOLD': 0.0003,
    'P1_TAKE_PROFIT': 0.3,
...
EBX_Config = apply_feature_selection(EBX_Config)
...
def prepare_p1_features(df: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    filtered_state_means, _ = kf_fast.filter(df[config['FEATURE_COLUMN']].dropna().values)
    smoothed_prices = pd.Series(filtered_state_means.squeeze(), index=df[config['FEATURE_COLUMN']].dropna().index)
    df["P1_KFTrendFast"] = smoothed_prices.shift(1)
    df["P1_KFTrendFast_Lagged"] = df["P1_KFTrendFast"].shift(1)
...
def generate_p1_signal_single(row, position, cooldown_over, config):
    ...
    if cooldown_over:
        if position == 0:
            if row["P1_KFTrendFast"] >= row["P1_UCM"] and row["P1_KAMA_Slope_abs"] > config['P1_ENTRY_KAMA_SLOPE_THRESHOLD']:
                signal = 1
            elif row["P1_KFTrendFast"] <= row["P1_UCM"] and row["P1_KAMA_Slope_abs"] > config['P1_ENTRY_KAMA_SLOPE_THRESHOLD']:
                signal = -1
        elif position == 1:
            if (row["P1_KFTrendFast_Lagged"] > row["P1_UCM_Lagged"] and 
                row["P1_KFTrendFast"] < row["P1_UCM"] and 
...
# ============================================================================
# STRATEGY 2 (KAMA + VOLATILITY WITH TRAILING STOP) - PRIORITY 2 FUNCTIONS
# ============================================================================
...
def process_single_day(file_path: str, day_num: int, temp_dir: pathlib.Path, config: dict) -> dict:
    ...
    try:
        ...
        # Add feature column only if not using Price
        if not config['USE_PRICE_FOR_INDICATORS']:
            required_cols.append('PB9_T1')
        ...
        p1_signals, df_with_p1 = generate_p1_signals(df, config)
        
        # 3. GENERATE PRIORITY 2 SIGNALS (KAMA + Volatility)
        p2_signals, df_with_p2 = generate_p2_signals(df, config)
...
def main(config):
    ...
    print(f"\nPriority 1: Kalman Filter Strategy ({config['FEATURE_COLUMN']} based)")
    ...
    print(f"\nPriority 2: KAMA + Volatility with Trailing Stop ({config['FEATURE_COLUMN']} based)")
