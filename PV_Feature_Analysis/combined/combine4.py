...
CONFIG = {
    ...
    'OUTPUT_FILE': 'managed_signals_4.csv',
    ...
    'P2_FEATURE_COLUMN': 'PB9_T1',  # Will be overridden if USE_PRICE_FOR_INDICATORS=True
    'P2_ATR_PERIOD': 25,
    'P2_STDDEV_PERIOD': 20,
    ...
    'P2_TP_LOW': 0.10,           # Low volatility TP
    'P2_TP_MEDIUM': 0.16,        # Medium volatility TP
    'P2_TP_HIGH': 0.29,          # High volatility TP
    'P2_STDDEV_TP_LOW': 0.05,    # Threshold for low TP
    'P2_STDDEV_TP_HIGH': 0.08,   # Threshold for high TP
    'P2_TRAILING_STOP_FROM_PEAK': 0.01,
    'P2_CONDITIONAL_SL': 0.1,    # Price movement against position to trigger check
    'P2_KAMA_SLOPE_THRESHOLD': 0.001,  # KAMA slope threshold for reversal confirmation
...
def apply_feature_selection(config):
    ...
    if config['USE_PRICE_FOR_INDICATORS']:
        ...
        config['P2_FEATURE_COLUMN'] = config['PRICE_COLUMN']
...
def apply_manager_logic(df: pd.DataFrame, p1_signals: np.ndarray, p2_signals: np.ndarray, 
    ...
    Rules:
    1. Daily reset
    2. Once P1 trades -> P2, P3, P4 locked for entire day
    3. Once P2 trades (if P1 hasn't) -> P3, P4 locked for entire day
    4. Once P3 trades (if P1, P2 haven't) -> P4 locked for entire day
    5. P4 can only trade if P1, P2, P3 haven't traded
    6. 15s universal cooldown between ANY signals
    7. Priority override: Higher priority can force exit of lower priority
    ...
    try:
        ...
        if not config['USE_PRICE_FOR_INDICATORS']:
            required_cols.extend([
                'PB9_T1',  # Used by P1, P2, P3, P4 if USE_PRICE_FOR_INDICATORS=False
            ])
        ...
        # ====================================================================
        p2_signals = generate_p2_signals(df, config, p4_kama_slope)
...
def main(config):
    ...
    print(f"\nPriority 2: Pure Momentum/Volatility with Dynamic TP ({config['P2_FEATURE_COLUMN']} based)")
    print(f"  - Volatility Filter: ATR>{config['P2_ATR_THRESHOLD']}, StdDev>{config['P2_STDDEV_THRESHOLD']}")
    print(f"  - 🎯 DYNAMIC TP: Low={config['P2_TP_LOW']}, Medium={config['P2_TP_MEDIUM']}, High={config['P2_TP_HIGH']}")
    print(f"  - 🚀 PURE TRAILING STOP: Exit when PnL drops {config['P2_TRAILING_STOP_FROM_PEAK']} from peak")
    print(f"  - 🔄 CONDITIONAL SL WITH REVERSAL: SL={config['P2_CONDITIONAL_SL']}, KAMA Slope Threshold={config['P2_KAMA_SLOPE_THRESHOLD']}")
    print(f"  - StdDev Thresholds: Low<{config['P2_STDDEV_TP_LOW']}, High>={config['P2_STDDEV_TP_HIGH']}")
    ...
    print(f"\nPriority 4: KAMA-based Simple Strategy ({config['P4_FEATURE_COLUMN']} based)")
