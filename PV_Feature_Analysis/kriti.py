...
Strategy_Config = {
    ...
    'OUTPUT_FILE': 'kriti_signals.csv',
    'TEMP_DIR': '/data/quant14/signals/temp_eama_kama_crossover',
    ...
    'RESAMPLE_PERIOD': 300,             # 300 seconds (5 min)
...
def _core_signal_logic(
    ...
    """
    Numba optimized core state machine.
    Enforces:
    1. Alternating signals (+1 -> -1 -> +1) or (-1 -> +1 -> -1)
    2. EOD Square off (Close on last candle)
    """
    n = len(eama_vals)
    ...
    for i in range(first_tradable_idx, n):
        ...
        # 3. Check EOD condition (Is this the last candle?)
        is_last_candle = (i == n - 1)
        
        # ============================================
        # EXIT LOGIC (Includes EOD Square Off)
        # ============================================
        if position == 1.0:
            # Must exit if Bearish Signal OR End of Day
            if bearish_crossover or is_last_candle:
                signals[i] = -1.0
                position = 0.0
                exit_candle_idx = i
            ...
            # Must exit if Bullish Signal OR End of Day
            if bullish_crossover or is_last_candle:
                signals[i] = +1.0
                position = 0.0
                exit_candle_idx = i
                continue # Done for this candle
        ...
        # ============================================
        # ENTRY LOGIC (Only if Flat and Not EOD)
        # ============================================
        if position == 0.0 and not is_last_candle:
...
def process_single_day(file_path: str, day_num: int, temp_dir: pathlib.Path, 
    ...
    try:
        ...
        if len(signals[signals_bool]) > 0:
            entry_signal = signals[signals_bool][0]
...
def generate_trade_reports_csv(output_file):
    ...
    for idx, row in signals_df.iterrows():
        ...
        if position is None:
            ...
            # We strictly expect the opposite signal here due to new logic
            if signal == -position:
                trades.append({
                    "day": entry["Day"],
