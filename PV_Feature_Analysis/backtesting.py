...
def backtest(df, initial_capital: float=100000, transaction_cost_rate:float=0.1, slippage: float = 0.0):
    ...
    for day, day_df in df.groupby("Day"):
        ...
        # --- NEW LOGIC: Attribute Realized PnL to Entry Interval ---
        session_start_time = day_df["Time"].iloc[0]
        
        # Extract arrays for faster iteration
        pos_values = day_df["Position"].values
        price_values = day_df["Price"].values
        time_values = day_df["Time"].values
        
        active_entry_price = 0.0
        active_entry_time = None
        active_direction = 0 # 1 for Long, -1 for Short
        
        for k in range(len(pos_values)):
            curr_pos = pos_values[k]
            prev_pos = pos_values[k-1] if k > 0 else 0
            curr_price = price_values[k]
            curr_time = time_values[k]
            
            # 1. Trade Entry (Flat -> Position)
            if prev_pos == 0 and curr_pos != 0:
                active_entry_price = curr_price
                active_entry_time = curr_time
                active_direction = np.sign(curr_pos)

            # 2. Reversal (Long -> Short or Short -> Long)
            elif prev_pos != 0 and curr_pos != 0 and np.sign(curr_pos) != np.sign(prev_pos):
                # Close Previous Trade
                exit_price = curr_price
                raw_pnl = (exit_price - active_entry_price) * active_direction * Factor
                trade_cost = (active_entry_price + exit_price) * transaction_cost_rate * Factor + (2 * slippage * Factor)
                net_pnl = raw_pnl - trade_cost
                
                # Add PnL to the bin where trade STARTED
                delta_mins = (active_entry_time - session_start_time).total_seconds() / 60
                bin_idx = int(delta_mins // interval_minutes)
                if 0 <= bin_idx < len(Interval_PnL):
                    Interval_PnL[bin_idx] += net_pnl
    ...
    Best5Days = np.argsort(Daily_PnL)[-3:][::-1]  # descending order
    WinningDays = np.argsort(Daily_PnL)[-51:][::-1]  # descending order
    BestDay = Best5Days[0]
    Worst5Days = np.argsort(Daily_PnL)[:3]       # ascending order
    LosingDays = np.argsort(Daily_PnL)[:41]       # ascending order
    WorstDay = Worst5Days[0]
    print(WinningDays, LosingDays)
    print("Best 5 Days:", Best5Days)
    print("Worst 5 Days:", Worst5Days)
    # days_to_plot = [208, 290, 239, 137, 343]
    days_to_plot = list(range(0,491))
    # match the feature names created in feature_analysis.py
    primary_cols = ["KFTrendFast", "SMA_10","SMA_45","UCM","EAMA","EAMA_Slow","HAMA","EhlersSuperSmoother","Upper_Wall","Lower_Wall"]
    secondary_cols = ["KAMA_Slope_abs","KAMA_Sum","EAMA_Slope","HAMA_Slope","EhlersSuperSmoother_Slope","H_Buy","H_Sell"]
    ...
    if len(days_to_plot) != 0:
        ...
        for day in days_to_plot:
            ...
            plot_pieces.append(day_df)
            
            gap_row = pd.DataFrame(np.nan, index=[0], columns=df.columns)
            plot_pieces.append(gap_row)
            
        plot_data = pd.concat(plot_pieces, ignore_index=True)
