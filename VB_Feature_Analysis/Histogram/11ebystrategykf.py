...
# --- Configuration ---
TEMP_DIR = "/data/quant14/signals/111temp_signal_processing"
OUTPUT_FILE = "/data/quant14/signals/11kf_trading_signals_EBY.csv"
...
def generate_signal(row, position, cooldown_over, strategy_params, state):
    ...
    if cooldown_over and time>=300:
        if position == 0:
            if row["KFTrendFast"] >= row["UCM"] and row["KAMA_Slope_abs_Lagged_2"] > 0.0007 and row["KAMA_Sum"] > 0.005 and row["ATR_High"] > 0.03:
                state["applyTP"] = True
                signal = -1
            elif row["KFTrendFast"] <= row["UCM"] and row["KAMA_Slope_abs_Lagged_2"] > 0.0007 and row["KAMA_Sum"] > 0.005 and row["ATR_High"] > 0.02:
                state["applyTP"] = True
                signal = 1
    return signal, state
...
def process_day(file_path: str, day_num: int, temp_dir: pathlib.Path, strategy_params) -> str:
    ...
    try:
        ...
        # --- ADDED: Save Day Number for Report Generation ---
        df['Day'] = day_num

        output_path = temp_dir / f"day{day_num}.csv"
        # --- UPDATED: Added 'Day' to final_columns ---
        final_columns = REQUIRED_COLUMNS + ['Day', 'Signal', 'Position', 'KFTrendFast', 'UCM', 'KAMA_Slope_abs']
...
def generate_trade_reports_csv(output_file):
    """Generates a trade book CSV from the signal file in the SAME directory."""
    import pandas as pd
    import os

    # Dynamic path: Save in the same directory as the output_file
...
def main(directory: str, max_workers: int, strategy_params):
    ...
    exclude_start = 600   # start excluding this day
    exclude_end = 790  # end excluding this day (inclusive)
