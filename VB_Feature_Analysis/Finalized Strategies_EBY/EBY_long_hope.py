...
# NEW: Path for the volatility statistics file
STATS_FILE = "/data/quant14/signals/daily_volatility_stats.csv" 
...
# Strategy 2: Medium Volatility (0.09 <= Sigma <= 0.12)
STRAT_2_PARAMS = {
    "entry_drop": 0.9,
    "static_sl": 999.0,
    "tp_activation": 1.0,
...
def extract_day_num_csv(filepath):
    match = re.search(r'day(\d+)(_stats)?\.csv', str(filepath))
    return int(match.group(1)) if match else -1
...
def process_day(file_path: str, day_num: int, temp_dir: pathlib.Path, strategy_params_cols) -> str:
    try:
        ...
        if np.any(mask_30min):
            ...
            if mu_condition:
                # CHECK STRATEGY 2 CONDITION (0.09 <= sigma <= 0.12)
                if 0.09 <= sigma <= 0.12:
                    can_trade_long = True
                    active_params = STRAT_2_PARAMS
                    strat_type = 2
                
                # CHECK STRATEGY 1 CONDITION (sigma > 0.12)
                elif sigma > 0.12:
                    can_trade_long = True
        ...
        stats_output_path = temp_dir / f"day{day_num}_stats.csv"
...
def main(directory: str, max_workers: int, strategy_params_cols):
    ...
    # --- 2. Merge Volatility Stats ---
    print("Merging volatility statistics...")
    stats_files = sorted(glob.glob(str(temp_dir_path / "*_stats.csv")), key=extract_day_num_csv)
    
    if stats_files:
        all_stats = []
        for sf in stats_files:
            try:
                all_stats.append(pd.read_csv(sf))
            except Exception as e:
                print(f"Error reading stats file {sf}: {e}")
        
        if all_stats:
            combined_stats = pd.concat(all_stats, ignore_index=True)
            combined_stats = combined_stats.sort_values("Day")
            combined_stats.to_csv(STATS_FILE, index=False)
            print(f"✅ Volatility Stats saved: {STATS_FILE}")
    else:
        print("⚠️ No volatility stats found to merge.")
...
# Final Capital                 : 111.32
# Total PnL                     : 11.32
# Total Transaction Cost         : 3.82
# Penalty Counts                : 0
# Final Returns                 : 11.32%
# CAGR                          : 10.1695%
# Annualized Returns            : 10.2233%
# Sharpe Ratio                  : 2.2205
# Calmar Ratio                  : 7.1256
...
# Losing Trades                 : 109
# Win Rate (%)                  : 43.23%
# Average Winning Trade         : 0.4716
# Average Losing Trade          : -0.1712
# Average Hold Period (seconds) : 8,197.57
# Average Hold Period (minutes) : 136.63
# ========================