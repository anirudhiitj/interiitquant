...
# Define days to completely exclude from processing (generate 0 signals)
BLACKLIST_DAYS = {
    87, 311, 273, 446, 5, 358, 14, 331, 136, 389, 504, 323, 96, 205, 230, 
    477, 456, 236, 181, 197, 372, 173, 28, 186, 367, 98, 410, 161, 107, 
    124, 208, 290, 239, 408, 137, 343, 108, 36, 448, 298, 85, 421
}
...
def main(directory: str, max_workers: int, strategy_params):
    ...
    # 2. Apply Blacklist (Day 60-79 + Specific Blacklisted Days)
    files = []
    skipped_count = 0
    for f in all_files:
        day_num = extract_day_num(f)
        
        # Check if day is in the numeric range OR in the specific set
        if (60 <= day_num <= 79) or (day_num in BLACKLIST_DAYS):
            skipped_count += 1
            # Skipping the file prevents signals from being generated for this day
            continue
            
        files.append(f)

    print(f"Found {len(all_files)} files. Blacklisted {skipped_count} files (Range 60-79 or Specific List). Processing {len(files)} files.")
