...
Strategy_Config = {
    ...
    'RESAMPLE_PERIOD': 90,              # 90 seconds for HA candles
...
def generate_signals(df: pd.DataFrame, config: dict) -> np.ndarray:
    """
    Generate signals based on EAMA/KAMA crossover with SMA filter on resampled data.
    
    LOOKAHEAD BIAS PREVENTION:
    1. No trading in first 30 minutes (volatility check period)
    2. Indicators calculated on tick data up to current candle close
    3. When resampling, use .last() to get the last value AT candle close
    4. All indicator values shifted by 1 period (use previous completed candle)
    5. Signal mapping uses ffill() to propagate signals forward (no future data)
    
    EOD square-off is ALLOWED (this is the only intentional lookahead)
    """
    ...
    # 1. Volatility Filter (on tick data)
    decision_window = config['DECISION_WINDOW']
    ...
    # 3. Calculate indicators on TICK data (point-in-time, no lookahead)
    # These indicators use recursive filters that only depend on past data
    ...
    # Calculate Heikin-Ashi (uses completed candle OHLC, no lookahead)
    ha_df = calculate_heikin_ashi(ohlc)
    
    # 5. Resample indicators and price to match candle timestamps
    # Using .last() gets the final value AT the candle close time (no future data)
    ...
    # 6. Calculate SMA on RESAMPLED Price (rolling window, no lookahead)
    ha_df['SMA'] = ha_df[config['PRICE_COLUMN']].rolling(
        window=config['SMA_PERIOD'], 
        min_periods=1
    ).mean()
    
    # 7. CRITICAL: SHIFT ALL DATA BY 1 PERIOD
    # At time T, we can only use data that was available at T-1 (previous completed candle)
    # This accounts for the fact that we don't know candle close values until the candle completes
    ha_df['EAMA_shifted'] = ha_df['EAMA'].shift(1)
    ha_df['KAMA_shifted'] = ha_df['KAMA'].shift(1)
    ha_df['Price_shifted'] = ha_df[config['PRICE_COLUMN']].shift(1)
    ha_df['SMA_shifted'] = ha_df['SMA'].shift(1)
    
    # 8. Mark the 30-minute cutoff (no trading during volatility check period)
    start_time = ha_df.index[0]
    cutoff_time = start_time + pd.Timedelta(seconds=decision_window)
    
    # 9. Generate signals on resampled data (AFTER 30 min, using SHIFTED data)
    resampled_signals = np.zeros(len(ha_df), dtype=np.float32)
    position = 0.0
    
    eama_vals = ha_df['EAMA_shifted'].values
    kama_vals = ha_df['KAMA_shifted'].values
    price_vals = ha_df['Price_shifted'].values
    sma_vals = ha_df['SMA_shifted'].values
    
    for i in range(1, len(ha_df)):
        if ha_df.index[i] <= cutoff_time:
            continue

        if any(np.isnan([eama_vals[i], kama_vals[i], 
                        eama_vals[i-1], kama_vals[i-1], sma_vals[i]])):
            continue

        eama_cur, kama_cur = eama_vals[i], kama_vals[i]
        eama_prev, kama_prev = eama_vals[i-1], kama_vals[i-1]
        price_cur, sma_cur = price_vals[i], sma_vals[i]
    ...
    # EOD SQUARE-OFF (ALLOWED - this is the only intentional lookahead)
    if position != 0.0:
        resampled_signals[-1] = -position
    
    # 10. Map signals back to original tick data
    # Using ffill() propagates the signal forward until the next signal
    # This is correct - once a signal is generated, it stays active until changed
...
def process_single_day_wrapper(args):
    """Wrapper for starmap to unpack arguments"""
    file_path, day_num, temp_dir, config = args
    return process_single_day(file_path, day_num, temp_dir, config)
...
def main():
    ...
    print("="*80)
    print("EAMA/KAMA CROSSOVER STRATEGY - FIXED LOOKAHEAD BIAS")
    print("="*80)
    ...
    print(f"  1. NO TRADING in first 30 minutes (volatility check period)")
    print(f"  2. HA candle data SHIFTED by 1 period (use previous completed candle)")
    print(f"\nIndicator Parameters:")
