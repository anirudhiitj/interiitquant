...
def backtest(df, initial_capital: float=100000, transaction_cost_rate:float=0.1, slippage: float = 0.0):
    ...
    for day, day_df in df.groupby("Day"):
        ...
        Adjusted_Signal = day_df["Position"].diff()
        day_df.loc[day_df.index, "Adjusted_Signal"] = Adjusted_Signal
        day_df.loc[day_df.index, "Signal"] = Adjusted_Signal
    ...
    primary_cols = ["KFTrendFast", "SMA_30","SMA_90_MA","TES","UCM"]
    secondary_cols = [ "Z-Score", "Z-Sigmoid", "Z-Sigmoid_MA", "Z-Sigmoid_Slope", "Breakout","KAMA_Slope_abs"]
...
if __name__ == "__main__":
    ddf=dask_cudf.read_csv("/data/quant14/signals/kf_trading_signals_EBY.csv")
