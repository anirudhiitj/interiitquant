...
OUTPUT_FILE = "/data/quant14/signals/trading_signals_EBX_combined.csv"
...
def prepare_derived_features(df: pd.DataFrame, strategy_params: dict) -> pd.DataFrame:
    ...
    df["KAMA_Sum_Vals"] = df["KAMA_Slope_abs"]
...
def generate_signal(row, position, cooldown_over, strategy_params, state):
    timestr=row["Time"]
    time=int(timestr[0:2]+timestr[3:5]+timestr[6:8])
    ...
    if cooldown_over:
        if position == 0:
            ...
            # else:
                ...
                #         signal = -1
            if time>600:
                if row['H_Buy'] > 1 and row["EAMA_Slope"] > 0.001 and row["EAMA_Slow"] - row["EAMA"] < 0.5:
                    signal = 1
                    state["trade_kalman"] = False
                    state["applyTP"] = False
                elif row["H_Sell"] > 1 and row["EAMA_Slope"] < -0.001 and row["EAMA"] - row["EAMA_Slow"] < 0.5:
                    signal = -1
                    state["trade_kalman"] = False
                    state["applyTP"] = False
                #     state["trade_kalman"] = False
...
def process_day(file_path: str, day_num: int, temp_dir: pathlib.Path, strategy_params) -> str:
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            if is_last_tick and position != 0:
                ...
                if not cooldown_over:
                    df.loc[i, "KAMA_Sum_Vals"] = 0
                if current_time - last_signal_time == COOLDOWN_PERIOD_SECONDS:
                    df["KAMA_Sum"] = df["KAMA_Sum_Vals"].rolling(window=5, min_periods=1).sum()
                row = df.iloc[i]
                ...
                # --- Exit Logic ---
                    ...
                    if state["trade_kalman"]:
                        ...
                        if price_diff >= tp and not applyTrailingSL:
        ...
        save_cols = ['Time', 'Price', 'Signal', 'Position', 'KFTrendFast', 'UCM', 
                     ...
                     'EhlersSuperSmoother', 'EhlersSuperSmoother_Slope',"EAMA","EAMA_Slow","EAMA_Slope"]
