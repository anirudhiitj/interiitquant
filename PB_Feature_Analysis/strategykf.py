...
# --- Configuration ---
TEMP_DIR = "/data/quant14/signals/kftemp_signal_processing"
OUTPUT_FILE = "/data/quant14/signals/kf_trading_signals_EBY.csv"
...
def prepare_derived_features(df: pd.DataFrame, strategy_params: dict) -> pd.DataFrame:
    ...
    # Convert the previously calculated numpy array 'ss' to Series for rolling ops
    ss_series = pd.Series(ss)

    # 1. Efficiency Ratio (ER) based on Super Smoother
    # Change over period vs Sum of absolute changes
    direction_ss = ss_series.diff(eama_period).abs()
    volatility_ss = ss_series.diff(1).abs().rolling(eama_period).sum()
    
    # Handle division by zero for ER
    er_ss = direction_ss / volatility_ss
    er_ss = er_ss.fillna(0).values

    # 2. Smoothing Constant (SC)
    fast_sc = 2 / (eama_fast + 1)
    slow_sc = 2 / (eama_slow + 1)
    # The SC adapts based on the "cleaner" Super Smoother trend
    ...
    for i in range(1, n):
        ...
        # Adaptively smooth the Super Smoother value
        eama[i] = eama[i-1] + c * (ss[i] - eama[i-1])

    df["EAMA"] = eama
    df["EAMA_Slope"] = pd.Series(eama).diff(1)
    df["EAMA_Slope_MA"] = df["EAMA_Slope"].rolling(window=30).mean()
    ...
    df["HA_High"] = ha_high
    df["HA_Low"] = ha_low
    df["HAMA"] = pd.Series(ha_close).rolling(180).mean().bfill().values
    df["HAMA_Slope"] = pd.Series(df["HAMA"]).diff(1)
    return df
    ...
    if cooldown_over:
        if position == 0:
            if row["KFTrendFast"]>=row["UCM"] and row["KAMA_Slope_abs"]>0.001:
                state["applyTP"] = True
                signal = -1
            if row["KFTrendFast"]>=row["UCM"] and row["KAMA_Slope_abs"]>0.0005:
                state["applyTP"] = True
                signal = -0.5
            ...
            #         signal = -1
                ...
                state["applyTP"] = True
                signal = 1
            elif row["KFTrendFast"]<=row["UCM"] and row["KAMA_Slope_abs"]>0.0005:
                state["applyTP"] = True
                signal = 0.5
            # elif row["KFTrendFast"]<=row["UCM"] and row["KAMA_Slope_abs"]>0.001:
            #     # state["applyTP"] = False
            ...
            if row["KFTrendFast"]<=row["UCM"] and row["KAMA_Slope_abs"]>0.001:
                ...
                signal = -position +0.5
            if row["EhlersSuperSmoother_Slope"] < -0.005:
                signal = -position
            # elif row["EhlersSuperSmoother_Slope"] < -0.007:
            #     signal = -position + 0.5
            # elif row["EhlersSuperSmoother_Slope"] > 0.007:
            #     signal = -position + 1
        elif position < 0:
...
def process_day(file_path: str, day_num: int, temp_dir: pathlib.Path, strategy_params) -> str:
    ...
    try:
                    ...
                    if state["applyTP"]:
                        ...
                        if price_diff >= tp3 and tp_stage < 3:
                            ...
                            if row["KAMA_Slope_abs"]<0.001:
                                ...
                                signal = -position + tp3pos
                                applyTrailingSL = True
                                trailing_sl_ref = price
                        ...
                        if applyTrailingSL and signal == 0:
                            if position > 0:
                                if price > trailing_sl_ref:
                                    trailing_sl_ref = price
                                price_diff = price - trailing_sl_ref
                            else:
                                if price < trailing_sl_ref:
                                    trailing_sl_ref = price
                                price_diff = trailing_sl_ref - price

                            if price_diff <= -trailing_sl:
                                signal = -position
                            
                        # if price_diff <= -worstSL:
                        #     signal = -position
            # --- Apply Signal ---
            if signal != 0:
        ...
        final_columns = REQUIRED_COLUMNS + ['Signal', 'Position', 'KFTrendFast', 'UCM', 'KAMA_Slope_abs','HAMA','HAMA_Slope','EhlersSuperSmoother','EhlersSuperSmoother_Slope']
