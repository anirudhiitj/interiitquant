...
OUTPUT_FILE = "/data/quant14/signals/hawkes_trading_signals_EBY.csv"
...
def prepare_derived_features(df: pd.DataFrame, strategy_params: dict) -> pd.DataFrame:
    ...
    df["HAMA_Slope"] = df["HAMA"].diff().fillna(0)
...
def generate_signal(row, position, cooldown_over, strategy_params, state):
    ...
    if cooldown_over:
        if position == 0:
            if row['H_Buy'] > 1 and row["EAMA_Slope"] > 0.001 and row["ATR_High"] < 0.025:
                signal = 1
            elif row['H_Sell'] > 1 and row["EAMA_Slope"] < -0.001 and row["ATR_High"] < 0.025:
                signal = -1
        elif position > 0:
...
def main(directory: str, max_workers: int, strategy_params):
    ...
    exclude_start = 500   # start excluding this day
    exclude_end = 500  # end excluding this day (inclusive)
