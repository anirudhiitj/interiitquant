...
def backtest(df, initial_capital: float=100000, transaction_cost_rate:float=0.02, slippage: float = 0.0):
    ...
    Worst5Days = np.argsort(Daily_PnL)[:1]

    days_to_plot = Worst5Days
    primary_cols = ["PB10_T4", "PB11_T4"]
    secondary_cols = ["PB5_T1", "PB7_T2", "PB7_T4"]
    ...
    if len(days_to_plot) != 0:
            ...
            if col in plot_data.columns:
                fig.add_trace(
                    go.Scatter(
                        ...
                        hovertemplate='<b>{col}</b><br>Value: %{y:,.2f}<br>Time: %{x}<extra></extra>'
                    ),
                    row=2, col=1, secondary_y=False
                )

        # # Additional columns on secondary y-axis
        # for col in secondary_cols:
        #     if col in plot_data.columns:
        #         series = plot_data[col].copy()
        ...
        #             else:
        #                 scaled = plot_data[col] * 0  # flat zero if constant column
        ...
        #                     line=dict(width=1.5),
        #                     visible=True,
        #                     hovertemplate='<b>{col}</b><br>Scaled Value: %{y:.2f}<br>Time: %{x}<extra></extra>'
        #                 ),
        #                 row=2, col=1, secondary_y=True
        #             )
        # Additional columns on secondary y-axis
...
if __name__ == "__main__":
    ddf=dask_cudf.read_csv("signal_predictions2.csv",usecols=["Time","Price","Position"])
