                        ...
                        if applyTrailingSL:
                            if position > 0:
                                if price > trailingSLRef:
                                    trailingSLRef = price
                                price_diff = price - trailingSLRef
                            elif position < 0:
                                if price < trailingSLRef:
                                    trailingSLRef = price
                                price_diff = trailingSLRef - price
                            if price_diff <= -trailingSL:
                                signal = -position
...
def main(directory: str, max_workers: int, strategy_params):
    ...
    # Prepare arguments for multiprocessing wrapper
    task_args = [
    ...
    try:
        ...
        # Using maxtasksperchild=10 to restart workers periodically 
        # and clear any GPU memory fragmentation/leaks
        ...
        with Pool(processes=max_workers, maxtasksperchild=10) as pool:
            ...
            # imap_unordered allows processing results as they complete
