...
def backtest(df, initial_capital: float=100000, transaction_cost_rate:float=0.1, slippage: float = 0.0):
    ...
    print(f"Average Winning Day PnL       : {Average_Winning_Day_PnL:,.4f}")
    print(f"Average Losing Day PnL        : {Average_Losing_Day_PnL:,.4f}")
    ...
    WinningDays = np.argsort(Daily_PnL)[-30:][::-1]  # descending order
    BestDay = Best5Days[0]
    Worst5Days = np.argsort(Daily_PnL)[:3]       # ascending order
    LosingDays = np.argsort(Daily_PnL)[:12]       # ascending order
    ...
    days_to_plot = Worst5Days.tolist()+Best5Days.tolist()
    ...
    if len(days_to_plot) != 0:
        ...
        fig.update_yaxes(title_text="Portfolio Value", row=1, col=1,secondary_y=True, gridcolor='#fdfff7')
        fig.update_yaxes(title_text="Price", row=1, col=1, secondary_y=False, gridcolor='#fdfff7')
        fig.update_yaxes(title_text="Signal", row=2, col=1, secondary_y=True, gridcolor='#fdfff7')
...
if __name__ == "__main__":
    df=pd.read_csv("/data/quant14/signals/11kf_trading_signals_EBX.csv")
    # ddf=ddf.drop(columns=["Position"])
