...
def generate_signal(row, position, cooldown_over, strategy_params, state):
    ...
    if cooldown_over and time>=60:
        if position == 0:
            if row["KFTrendFast"] >= row["UCM"] and row["KAMA_Slope_abs_Lagged_2"] > 0.0007 and row["KAMA_Sum"] > 0.005 and row["ATR_High"] > 0.025:
                state["applyTP"] = True
                signal = -1
            ...
            #     else:
            #         signal = -1
            elif row["KFTrendFast"] <= row["UCM"] and row["KAMA_Slope_abs_Lagged_2"] > 0.0007 and row["KAMA_Sum"] > 0.005 and row["ATR_High"] > 0.025:
                state["applyTP"] = True
                signal = 1
            # elif row["KFTrendFast"]<=row["UCM"] and row["KAMA_Slope_abs"]>0.001:
            #     # state["applyTP"] = False
